...
class DTED(nn.Module):
    ...
    def transform_code(self, data, code, mode, fr=None, to=None):

        random_integer = np.random.randint(0, 4)

        if random_integer == 0 or random_integer == 1:
            ...
            # 进行一个水平翻转
            return torch.flip(code, (1,))

        elif random_integer == 3:
            matrix_shape = code.shape
            # 生成50个随机索引，用于放置噪点
            random_indices = torch.randint(0, matrix_shape[0], (50,))
            # 生成随机噪点并添加到矩阵中
            noise_points = torch.rand(50, matrix_shape[1])
            code[random_indices] = noise_points
            return code
    ...
    def forward(self, data, loss_functions=None):
        ...
        output_dict = {
            ...
            'z_gaze_enc': ze1_g_a,
            'z_head_enc': ze1_h_a,
            'canon_z_gaze_a': self.transform_code(data, ze1_g_a, 'gaze', fr='a'),
            'canon_z_head_a': self.transform_code(data, ze1_h_a, 'head', fr='a'),
        }
        if 'R_gaze_b' not in data:
            return output_dict

        if not is_inference_time:
            output_dict['canon_z_gaze_b'] = self.transform_code(data, ze1_g_b, 'gaze', fr='b')
            output_dict['canon_z_head_b'] = self.transform_code(data, ze1_h_b, 'head', fr='b')

        # Rotate codes
        # transform codes
        zd1_g_b = self.transform_code(data, ze1_g_a, 'gaze', fr='a', to='b')
        zd1_h_b = self.transform_code(data, ze1_h_a, 'head', fr='a', to='b')

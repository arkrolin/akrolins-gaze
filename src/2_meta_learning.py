...
class Reptile(MAML):
    ...
    def train_(self, steps_outer, steps_inner=1, lr_inner=0.01, lr_outer=0.001,
              disable_tqdm=False):
        # train的重写
        self.lr_inner = lr_inner
        print('\nBeginning meta-learning for k = %d' % self.k)
        ...
        for i in tqdm(range(steps_outer), disable=disable_tqdm):
            ...
            # 使用使用一个近似的一步梯度下降来更新模型参数
            for param, param_copy in zip(self.model.parameters(), self.meta_model.parameters()):
                param.data.sub_(lr_outer * (param.data - param_copy.data))

            # 仅计算损失而不进行完整的梯度反向传播
            new_task_loss = forward(self.meta_model, test_data, train_data=train_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--meta-learner', type=str, default='Meta-SGD',
